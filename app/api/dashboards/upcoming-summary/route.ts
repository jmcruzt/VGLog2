...
export async function GET() {
  ...
  const { rows: gamePassByYear } = await client.execute("SELECT release_year as label, COUNT(*) as count FROM games WHERE status='upcoming' AND is_game_pass=1 AND release_year IS NOT NULL GROUP BY release_year ORDER BY release_year DESC");
